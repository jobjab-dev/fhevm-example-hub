---
id: user-decrypt-single-value
title: User Decrypt Single
category: decryption
...
This example demonstrates how to implement **User Decrypt Single** using FHEVM.

| Property | Value |
|----------|-------|
| **Level** | 🟡 Intermediate |
| **Category** | decryption |
| **Key** | `user-decrypt-single-value` |

## 🔓 User Decryption (Re-encryption)

User decryption allows a specific user to decrypt a value off-chain:

```mermaid
sequenceDiagram
    participant U as User
    participant C as Contract
    participant R as Relayer/Gateway
    U->>C: Call function
    C->>C: FHE.allow(handle, user)
    U->>R: Request re-encryption
    R-->>U: Decrypted value
```

## ⚠️ Required: FHE.allow

If you forget `FHE.allow(handle, user)`, decryption will **FAIL**.

## 🧠 Key FHE Concepts Used

- `FHE.allow for user permissions`
- `Re-encryption via Gateway`
- `Selective disclosure`

## 📝 Contract Implementation

`contracts/UserDecryptSingleValue.sol`
...
`test/UserDecryptSingleValue.ts`
...
npx jobjab-fhevm-examples user-decrypt-single-value ./my-user-decrypt-single-value
