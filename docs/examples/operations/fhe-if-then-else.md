...
category: operations
tags: [operations, logic, intermediate]
description: Shows conditional operations on encrypted values
---

# FHE If-Then-Else

> Shows conditional operations on encrypted values

## Overview

This example demonstrates how to implement **FHE If-Then-Else** using FHEVM.

| Property | Value |
|----------|-------|
| **Level** | 🟡 Intermediate |
| **Category** | operations |
| **Key** | `fhe-if-then-else` |

## 🔀 Encrypted Branching with FHE.select

Since you can't use `if` statements on encrypted booleans, use `FHE.select`:

```solidity
// Instead of:
if (encryptedCondition) { result = a; } else { result = b; }

// Use:
result = FHE.select(encryptedCondition, a, b);
```

```mermaid
flowchart LR
    A[ebool condition] --> B{FHE.select}
    C[euint ifTrue] --> B
    D[euint ifFalse] --> B
    B --> E[euint result]
```
...
- `FHE.select for branching`
- `Cannot use if on ebool`
- `Encrypted multiplexer pattern`

## 📝 Contract Implementation

`contracts/FHEIfThenElse.sol`
...
npx jobjab-fhevm-examples fhe-if-then-else ./my-fhe-if-then-else

# Or with the CLI
npm run create fhe-if-then-else ./my-fhe-if-then-else
```

Then build and test:

```bash
cd ./my-fhe-if-then-else
npm install
npm run compile
npm run test
```
