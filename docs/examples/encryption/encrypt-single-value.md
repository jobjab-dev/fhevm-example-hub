---
id: encrypt-single-value
title: Encrypt Single Value
category: encryption
tags: [encryption, beginner]
description: Demonstrates FHE encryption mechanism and common pitfalls
---

# Encrypt Single Value

> Demonstrates FHE encryption mechanism and common pitfalls

## Overview

This example demonstrates how to implement **Encrypt Single Value** using FHEVM.

| Property | Value |
|----------|-------|
| **Level** | 🟢 Beginner |
| **Category** | encryption |
| **Key** | `encrypt-single-value` |

## 🔐 Understanding Encryption Flow

```mermaid
flowchart TD
    A[Client: Generate Encrypted Input] --> B[Bound to Contract + User]
    B --> C[Send TX with handle + proof]
    C --> D[Contract: FHE.fromExternal]
    D --> E{Proof Valid?}
    E -->|Yes| F[Store euint handle]
    E -->|No| G[Revert]
```

## ⚠️ Common Pitfall

If the transaction sender doesn't match the proof signer, it will **REVERT**.

## 🧠 Key FHE Concepts Used

- `Client-side encryption with fhevm.js`
- `Input proofs and binding`
- `FHE.fromExternal validation`

## 📝 Contract Implementation

`contracts/EncryptSingleValue.sol`
...
npx jobjab-fhevm-examples encrypt-single-value ./my-encrypt-single-value

# Or with the CLI
npm run create encrypt-single-value ./my-encrypt-single-value
```

Then build and test:

```bash
cd ./my-encrypt-single-value
npm install
npm run compile
npm run test
```
